...
function normalizeBackground(value) {
  ...
  const imageOpacity = typeof value?.imageOpacity === "number" && Number.isFinite(value.imageOpacity) ? Math.max(0, Math.min(1, value.imageOpacity)) : 0.42;
  return { background, customBackground, imageOpacity };
}
function normalizeDesignProfiles(profiles) {
  ...
  for (const value of Array.isArray(profiles) ? profiles.slice(0, 8) : []) {
    ...
    result.push({ name, theme: ["aurora", "graphite", "midnight", "cyberpunk"].includes(value.theme) ? value.theme : "aurora",
